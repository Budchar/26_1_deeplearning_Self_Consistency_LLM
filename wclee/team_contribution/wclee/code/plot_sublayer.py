...
# summary JSON은 contribution 내부에, raw는 메인 프로젝트에서 읽음
ROOT = CONTRIB.parent.parent  # hallucination_detect/
sys.path.insert(0, str(ROOT))

raw     = ROOT / "results" / "raw"
fig_dir = CONTRIB / "plots"
...
# ── Summary table ─────────────────────────────────────────────
print(f"\n{'Model':<25} {'n_L':>4} {'Exp06L':>6} {'BestMLP':>8} {'attn@BML':>10} {'mlp@BML':>10} {'gap':>8} {'Driver':>10}")
print("-" * 80)
for mkey in sorted(models_present):
    d    = data[mkey]
    n_L  = d["n_layers"]
    e06l = d.get("exp06_best_layer", 0)
    # best_mlp_layer 기준으로 driver 판정 (더 명확한 비교 지점)
    bml  = d.get("best_mlp_layer", e06l)
    aa   = d["attn_stream_auroc"][bml] if bml < n_L else 0
    ma   = d["mlp_stream_auroc"][bml] if bml < n_L else 0
    gap  = ma - aa
    drv  = "MLP" if gap > 0.02 else ("Attn" if gap < -0.02 else "Both")
    print(f"  {mkey:<23} {n_L:>4} {e06l:>6} {bml:>8} {aa:>10.3f} {ma:>10.3f} {gap:>+8.3f} {drv:>10}")
